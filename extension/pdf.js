import {splitPdfText} from './pdf-chunks.js';
import * as pdfjs from './vendor/pdf.mjs';
pdfjs.GlobalWorkerOptions.workerSrc=chrome.runtime.getURL('vendor/pdf.worker.mjs');
const $=s=>document.querySelector(s);let doc,num=1,text='',busy=false;const cache=new Map(),partials=new Map();
...
async function show(){busy=true;controls();try{
 ...
 $('#page').textContent=`${num} / ${doc.numPages}`;$('#translation').textContent=cache.get(num)||partials.get(num)?.join('\n\n')||'点击「翻译本页」，查看对应译文。';
...
$('#translate').onclick=async()=>{busy=true;controls();$('#status').textContent='正在翻译本页…';try{
 const chunks=splitPdfText(text),out=partials.get(num)||[];
 partials.set(num,out);
 $('#translation').textContent=out.join('\n\n')||'正在生成译文…';
 for(let i=out.length;i<chunks.length;i++) {
  $('#status').textContent=`正在翻译本页：${i+1} / ${chunks.length} 段`;
  const r=await chrome.runtime.sendMessage({type:'translate',text:chunks[i]});if(r.error)throw Error(r.error);
  out.push(r.text);$('#translation').textContent=out.join('\n\n');
 }
 const result=out.join('\n\n');cache.set(num,result);$('#translation').textContent=result;$('#status').textContent='本页翻译完成';
 }catch(e){if(!partials.get(num)?.length)$('#translation').textContent='本页尚无已完成译文，请重试。';$('#status').textContent=e.message+'。再次点击「翻译本页」将从未完成处继续。';}finally{busy=false;controls();}};
