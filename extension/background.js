...
chrome.runtime.onMessage.addListener((m,sender,reply)=>{
 ...
 (async()=>{
  await secured;
  const s=validate({...defaults,...(await chrome.storage.local.get('settings')).settings});
  ...
  try {return {text:await translate(s,m.text)};} finally {clearInterval(keepAlive);release();}
